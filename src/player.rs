...
fn process_mouse_rotation(window: &Window, player: &mut Player, last_mouse_x: &mut Option<f32>) {
    ...
    if let Some((mouse_x, _)) = window.get_mouse_pos(MouseMode::Pass) {
        if let Some(prev_x) = *last_mouse_x {
            let mut delta_x = mouse_x - prev_x;

            // Si el cursor alcanza el borde lateral de la ventana, mantener rotación continua suave
            if mouse_x >= win_w - 5.0 && delta_x >= 0.0 {
                delta_x = 10.0;
            } else if mouse_x <= 5.0 && delta_x <= 0.0 {
                delta_x = -10.0;
            }

            // Ignorar saltos bruscos o irreales de posición (ej. cambio de ventana o recaptura)
            if delta_x.abs() < 150.0 {
                player.a += delta_x * MOUSE_SENSITIVITY;
            }
        }
